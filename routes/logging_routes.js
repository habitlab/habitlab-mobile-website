...
get_domain_name = function(domain, from) {
  if (from == ANDROID && domain == "com.google.android.gm" ||
      from == BROWSER && domain == "mail.google.com") {
        //Gmail is a common example where the domains don't associate nicely.
        return "gmail"
  }
  // Remove TLDS
  const subdomains = domain.split(".").filter(x => !TLDs.includes(x))
  if (subdomains.length == 0) {
    // I don't know what's important, so I'm better off just returning the
    // original domain.
    return domain
  }
  if (from == ANDROID) {
    //Return first interesting part of subdomains (android packages are backwards)
    return subdomains[0]
  } else {
    //Return last interesting part of subdomain
